...
В модалке редактирования товара (открывается из админ-дашборда по иконке карандаша) клик по любому `<input>` приводил к перерендеру всей модалки: фокус терялся, введённые символы откатывались к исходным значениям из БД, отредактировать товар было невозможно.

Причина — коллизия имён `data-action="edit-item"`. Одно и то же значение использовалось:
- на кнопке открытия модалки (`public/js/ui.js:313`), которую глобальный click-делегатор маршрутизирует в `handleEditItem(id)` (`public/js/app.js:90`);
- на самой `<form>` внутри модалки (`public/js/ui.js:387`) — она нужна только для submit-делегатора.

При клике по input внутри формы `target = e.target.closest('[data-action]')` поднимался по DOM и натыкался на форму раньше, чем на `#modal-overlay`. Action оказывался `"edit-item"`, срабатывала ветка `app.js:90`, и `handleEditItem` повторно тянул товар из Firestore и переписывал `overlay.innerHTML`. Старая нода input уничтожалась, новая монтировалась без фокуса и с исходным `value`.

### Что сделано

- `public/js/ui.js:387` — форма редактирования товара переименована с `data-action="edit-item"` на `data-action="update-item"`. Кнопку открытия (`ui.js:313`) трогать не нужно — она и должна оставаться `edit-item`.
- `public/js/app.js:138` — submit-делегатор обновлён: `if (action === 'update-item') await this.handleSaveItem(form);`.

Других форм с таким же конфликтом нет: `create-item`, `save-user-profile`, `grant-item` не имеют click-веток с теми же именами — у них только submit-ветки.

### Проверка

1. Зайти админом → раздел «Админ» → у любого товара нажать карандаш.
2. Кликнуть в поле «Название» — фокус остаётся, поле редактируется. Поменять значение, кликнуть в «Описание» — изменения в «Название» сохраняются.
3. Нажать «Сохранить изменения» — submit отрабатывает, модалка закрывается, дашборд перерисовывается.
4. Регрессия: повторно открыть модалку через карандаш — должна открываться.
